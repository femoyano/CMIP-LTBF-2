...
CostTrans <- function(pars_optim, pars_default = pars_default, site_data = site_data, C_obs = C_obs, input_spin, input_trans) {
  ...
  out <- StartRun(input = input_trans, site_data = site_data, pars_default = pars_default, pars_new = pars_optim, initial_state = initial, tsave = month)
  ...
  sqres <- mean(res^2)
  if(sqres > 9999 | out[1,'C_P'] < 0) sqres <- 9999
